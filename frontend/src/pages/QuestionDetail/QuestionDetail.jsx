...
export default function QuestionDetail() {
  ...
  const loadQuestion = async () => {
    ...
    try {
      ...
      const loadedQuestion = result?.question || result?.data?.question || null;

      const loadedAnswers = result?.answers || result?.data?.answers || [];

      setQuestion(loadedQuestion);

      setAnswers(Array.isArray(loadedAnswers) ? loadedAnswers : []);
  ...
  const handleSubmitAnswer = async (event) => {
    ...
    if (isWeakDraft) {
      setMessage("Improve your answer before submitting.");
    ...
    try {
      ...
      setMessage(getErrorMessage(error, "Failed to post answer."));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className={styles.answers}>
          {answers.length === 0 ? (
            <div className={styles.emptyBox}>
              ...
              <p>Share your knowledge and earn reputation points.</p>
            </div>
          ) : (
            answers.map((answer) => (
              <article key={answer.id} className={styles.answerCard}>
      ...
      <aside className={styles.side}>
        ...
        {relatedQuestions.length === 0 ? (
          ...
          relatedQuestions.map((item) => {
            const id = item.questionHash || item.id;
